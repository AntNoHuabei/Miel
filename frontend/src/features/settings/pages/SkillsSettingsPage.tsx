      ...
      <Tabs className="bm-skills-tabs" activeKey={tab} onChange={(key) => setTab(key as 'installed' | 'skillhub')} items={[
        {
          ...
          children: <div className="bm-skills-tab-panel">
            <div className="bm-skills-toolbar"><Text type="secondary">管理 Miel 可加载的技能</Text><div><Tooltip title="刷新技能列表"><Button type="text" icon={<ReloadOutlined />} aria-label="刷新技能列表" onClick={() => void loadInstalled()} /></Tooltip><Button type="primary" icon={<UploadOutlined />} loading={importing} onClick={() => void importLocal()}>导入本地 Skill</Button></div></div>
