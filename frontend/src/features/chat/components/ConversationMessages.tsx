...
export function AgentProcess({ phase, process, tools, skillProgress }: { phase: AgentPhase; process: AgentProcessStep[]; tools: AgentToolCall[]; skillProgress: { skill: string; stage: string; message?: string; state: string } | null }) {
  ...
  const label = phase === 'compacting' ? '正在整理上下文' : phase === 'thinking' ? '正在思考' : phase === 'tool' ? '正在使用工具' : phase === 'responding' ? '正在组织回复' : phase === 'done' ? '处理完成' : phase === 'error' ? '处理失败，对话已结束' : '正在等待模型响应'
  return (
    <div className="bm-agent-process">
      ...
      {phase !== 'compacting' && skillProgress && <div className="bm-tool-detail"><Text type="secondary">{skillProgress.skill} · {skillProgress.message || skillProgress.stage}</Text></div>}
      {phase !== 'compacting' && process.map((step) => {
